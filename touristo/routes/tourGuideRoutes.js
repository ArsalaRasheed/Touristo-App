...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: tourGuides });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error fetching tour guides', error: error.message });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: tourGuide });
  } catch (error) {
    console.error('Error fetching tour guide:', error);
    res.status(500).json({ success: false, message: 'Error fetching tour guide', error: error.message });
  }
});
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, data: newTourGuide });
  } catch (error) {
    console.error('Error creating tour guide:', error);
    res.status(500).json({ success: false, message: 'Error creating tour guide', error: error.message });
  }
});
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: updatedTourGuide });
  } catch (error) {
    console.error('Error updating tour guide:', error);
    res.status(500).json({ success: false, message: 'Error updating tour guide', error: error.message });
  }
});
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error deleting tour guide', error: error.message });
  }
});

...
const SearchScreen = () => {
  ...
  const [hosts, setHosts] = useState([]); // State for hosts
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedHosts = hostsData.data?.hosts || [];
  ...
  const filteredHosts = hosts
    .filter(host => 
      (host.company_name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (host.description && host.description.toLowerCase().includes(searchQuery.toLowerCase()))
    )
    .sort((a, b) => b.rating_score - a.rating_score); // Sort by rating descending
    ...
    return (
      <div className="min-h-screen bg-[color:var(--bg-primary)] text-[color:var(--text-primary)] p-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-[color:var(--text-secondary)]">Searching packages...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[color:var(--bg-primary)] text-[color:var(--text-primary)] p-4 flex items-center justify-center">
        <div className="text-center p-6 bg-[color:var(--surface-primary)] rounded-xl border border-[color:var(--border-primary)] max-w-md">
          <h2 className="text-xl font-bold mb-2 text-red-500">Error Loading Data</h2>
          <p className="text-[color:var(--text-secondary)] mb-4">Failed to load packages: {error}</p>
          <button 
  ...
  return (
    <div className="min-h-screen bg-[color:var(--bg-primary)] text-[color:var(--text-primary)] p-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 text-[color:var(--text-primary)]">Find Your Perfect Trip</h1>
        ...
        <div className="space-y-8">
          {/* Package Results */}
          <div>
            <h2 className="text-xl font-bold mb-4 text-[color:var(--text-primary)]">Package Results</h2>
            
            {filteredPackages.length === 0 ? (
              <div className="text-center py-6">
                <div className="text-5xl mb-4">🔍</div>
                <h3 className="text-xl font-bold mb-2 text-[color:var(--text-primary)]">No packages found</h3>
                <p className="text-[color:var(--text-secondary)] mb-6">Try adjusting your search or check back later for new listings.</p>
              </div>
            ) : (
                    ...
                    <div className="bg-[color:var(--surface-primary)] rounded-xl p-4 flex items-center gap-4 hover:bg-[color:var(--surface-secondary)] transition border border-[color:var(--border-primary)] relative">
                      ...
                      <div className="flex-grow">
                        <div className="flex items-center mb-1">
                          ...
                          <span className="font-medium text-sm">{pkg.avg_rating || pkg.rating || 0}</span>
                        </div>
                        <h3 className="font-bold text-[color:var(--text-primary)] text-sm">{pkg.title || 'Package Title'}</h3>
                        <p className="text-[color:var(--text-secondary)] text-xs">{pkg.host_name || 'Host'}</p>
                        <div className="flex items-center mt-2">
                          <span className="font-bold text-[color:var(--accent-primary)] text-sm">PKR {parseInt(pkg.price || 0).toLocaleString()}</span>
                          <span className="mx-2 text-xs">•</span>
                          <span className="text-xs text-[color:var(--text-secondary)]">{pkg.duration_days || 0} days</span>
                          <span className="mx-2 text-xs">•</span>
                          <span className="text-xs text-[color:var(--text-secondary)]">{pkg.group_size || '2-6 people'}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
          ...
          {/* Tour Companies for this destination */}
          <div>
            <h2 className="text-xl font-bold mb-4 text-[color:var(--text-primary)]">Tour Companies for "{searchQuery}"</h2>
            
            {filteredHosts.length === 0 ? (
              <div className="text-center py-6">
                <h3 className="text-lg font-medium text-[color:var(--text-secondary)]">No tour companies found for this search</h3>
              </div>
            ) : (
                  ...
                  <Link to={`/host-profile/${host.id}`} key={host.id}>
                    <div className="bg-[color:var(--surface-primary)] rounded-xl p-4 hover:bg-[color:var(--surface-secondary)] transition border border-[color:var(--border-primary)] relative">
                      ...
                      <div className="flex items-center">
                        <div className="w-12 h-12 rounded-full bg-[color:var(--accent-primary)] flex items-center justify-center text-[color:var(--nav-text)] font-bold mr-3">
                          {host.company_name?.charAt(0)}
                        </div>
                        <div className="flex-grow">
                          <div className="flex items-center">
                            {host.verified && (
                              <span className="bg-[color:var(--accent-primary)] text-[color:var(--nav-text)] px-2 py-1 rounded-full text-xs flex items-center mr-2">
                            ...
                            <h3 className="font-bold text-[color:var(--text-primary)]">{host.company_name}</h3>
                          </div>
                          <p className="text-sm text-[color:var(--text-secondary)]">{host.location || 'Pakistan'}</p>
                          <div className="flex items-center mt-1">
                            <svg className="w-4 h-4 text-[color:var(--accent-primary)] mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                            <span className="text-sm">{host.rating_score?.toFixed(1) || 0}</span>
                            <span className="mx-2">•</span>
                            <span className="text-sm">{host.trips_completed || 0} trips</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {host.ranking_badge && (
                        <div className="mt-3">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                            host.ranking_badge === 'Top Rated' 
                              ? 'bg-yellow-100 text-yellow-800' 
                              : host.ranking_badge === 'Highly Recommended' 
                                ? 'bg-blue-100 text-blue-800' 
                                : host.ranking_badge === 'Rising Host' 
                                  ? 'bg-green-100 text-green-800' 
